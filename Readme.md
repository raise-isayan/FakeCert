Burp suite Certificate Change Tool
=============
Language/[Japanese](Readme-ja.md)

This tool is a tool for changing certificates and adding information that is not included, which is dynamically generated by Burp Suite, a product of PortSwigger.
...
1. change of CN
2. specify the SerialNumber
3. specifying the certificate's expiration date
4. change or add a SAN (Subject Alternative Name).
5. add an OCSP URI.
...
Navigate to the folder with the Burp suite jar file and run it with the -javaagent command line option (before the -jar option).
